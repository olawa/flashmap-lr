...
FASTA/FASTQ-to-SAM CLI adapter is now available for smoke tests and a read-only,
mmap-backed FlashMap v13 `.fmi` adapter is available for real references.
Score-aware endpoint clipping, endpoint attachment, seed tier escalation, and
full FlashMap differential testing remain before claiming production parity.
...
For a persistent FlashMap v13 index, use the index directly; the reference
